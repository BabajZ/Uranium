...
public
class EntityUtil2 implements Util {
    ...
    public static void attackEntity(final Entity entity, final boolean packet, final boolean swingArm) {
        if (packet) {
            ...
            EntityUtil2.mc.playerController.attackEntity(EntityUtil.mc.player, entity);
        }
        if (swingArm) {
    ...
    public static
    boolean isAboveLiquid ( final Entity entity ) {
        if ( entity == null ) {
            return false;
        }
    ...
    public static
    double[] forward ( final double speed ) {
