...
public class BlockUtil
    ...
    public static
    boolean placeBlockNotRetarded ( BlockPos pos , EnumHand hand , boolean rotate , boolean packet , boolean altRotate ) {
        EnumFacing side = BlockUtil.getFirstFacing ( pos );
        ...
        if ( rotate ) RotationUtil.faceVector ( altRotate ? new Vec3d ( pos ) : hitVec , true );
        BlockUtil.rightClickBlock ( neighbour , hitVec , hand , opposite , packet );
        BlockUtil.mc.player.swingArm ( EnumHand.MAIN_HAND );
        return true;
    }
    ...
    public static void damageBlock(BlockPos position, EnumFacing facing, boolean packet, boolean rotations) {
        ...
        if (packet) {
            BlockUtil.mc.player.connection.sendPacket(new CPacketPlayerDigging(net.minecraft.network.play.client.CPacketPlayerDigging.Action.START_DESTROY_BLOCK, position, facing));
            BlockUtil.mc.player.connection.sendPacket(new CPacketPlayerDigging(net.minecraft.network.play.client.CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, position, facing));
