...
public class BoatFly extends Module {
    ...
    private void NCPPacketTrick() {
        packetCounter = 0;
        Util.mc.player.getRidingEntity().dismountRidingEntity();
        Entity l_Entity = Util.mc.world.loadedEntityList.stream().filter(p_Entity -> p_Entity instanceof EntityBoat).min(Comparator.comparing(p_Entity -> Util.mc.player.getDistance(p_Entity))).orElse(null);
        if (l_Entity != null) {
            Util.mc.playerController.interactWithEntity(Util.mc.player, l_Entity, EnumHand.MAIN_HAND);
        }
    }
    ...
    public void onSendPacket(PacketEvent.Send event) {
        ...
        if (Util.mc.world != null && Util.mc.player != null) {
            if (Util.mc.player.getRidingEntity() instanceof EntityBoat) {
                if (bypass.getValue() && event.getPacket() instanceof CPacketInput && !Util.mc.gameSettings.keyBindSneak.isKeyDown() && !Util.mc.player.getRidingEntity().onGround) {
                    ++packetCounter;
                    if (packetCounter == 3) {
                        NCPPacketTrick();
                    }
        ...
        if (placeBypass.getValue() && event.getPacket() instanceof CPacketPlayerTryUseItemOnBlock
                && Util.mc.player.getHeldItemMainhand().getItem() instanceof ItemBoat || Util.mc.player.getHeldItemOffhand().getItem() instanceof ItemBoat) {
            event.setCanceled(true);
        }
    }
