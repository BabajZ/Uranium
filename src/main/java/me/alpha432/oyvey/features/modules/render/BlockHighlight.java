...
class BlockHighlight extends Module {
    ...
    private final Setting < Integer > cBlue = register ( new Setting ( "OL-Blue" , 0 , 0 , 255 , v -> customOutline.getValue ( ) && outline.getValue ( ) ) );
    private final Setting < Integer > cAlpha = register ( new Setting ( "OL-Alpha" , 255, 0 , 255 , v -> customOutline.getValue ( ) && outline.getValue ( ) ) );

    public
    BlockHighlight ( ) {
        super ( "BlockLight" , "Highlights the block u look at." , Category.RENDER , false , false , false );
    }
