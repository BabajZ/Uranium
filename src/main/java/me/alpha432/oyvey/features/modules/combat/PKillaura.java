...
import me.alpha432.oyvey.util.Client.*;
import me.alpha432.oyvey.util.Player.DamageUtil;
...
public class PKillaura
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue().booleanValue()) {
            if (this.stay.getValue().booleanValue() && target != null) {
                OyVey.ProtationManager.lookAtEntity(target);
            }
            this.doKillaura();
        }
    }
    ...
    private void doKillaura() {
        ...
        int n = this.delay.getValue() == false  && this.teekaydelay.getValue() == false ? 0 : (wait = (int)((float) DamageUtil.getCooldownByWeapon((EntityPlayer)Killaura.mc.player) * (this.tps.getValue() != false ? OyVey.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue().booleanValue()) {
            OyVey.positionManager.setPositionPacket(Killaura.target.posX, EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double)target.getEyeHeight(), Killaura.target.posZ, true, true, this.lagBack.getValue() == false);
        }
